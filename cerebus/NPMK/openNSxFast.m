...
try
    ...
    elseif or(strcmpi(NSx.MetaTags.FileTypeID, 'NEURALCD'), strcmpi(NSx.MetaTags.FileTypeID, 'BRSMPGRP'))
        if flagOneSamplePerPacket
            if flagSegment
                ...
                % add into NSx structure
                fseek(FID,f.EOexH + 1,'bof'); % + byte (header)
                tsVal = fread(FID,1,timestampType);
                fprintf('DEBUG REF: f.EOexH=%d, Timestamp (offset %d)=%d\n', f.EOexH, f.EOexH+1, tsVal);
                NSx.MetaTags.Timestamp = tsVal;
    ...
    if flagReadData
        ...
        for currSegment = requestedSegments(1):requestedSegments(2)
            ...
            % (Assuming flagOneSamplePerPacket is true here due to block context)
            startOffsetBytes = (segmentStartDataPoint(currSegment) - 1) * packetSize;
            fseek(FID, startOffsetBytes, 'cof');
            
            % --- OPTIMIZATION START by Antigravity (Bulk Read) ---
            % Instead of iterative fread with skip, read ALL bytes and parse in RAM.
            
            % f.BOData points to the START OF DATA for the first packet.
            % We want to read WHOLE PACKETS (Header + Data).
            % So we must rewind to the start of the packet header.
            % headerSize = 1 + timestampSize + 4
            headerOffset = 1 + timestampSize + 4;
            fseek(FID, -headerOffset, 'cof');
            
            % Calculate total bytes to read for this segment
            % numOutputPoints = floor(segmentDataPoints(currSegment)/requestedSkipFactor);
            % But we need to read enough packets to satisfy the skip factor.
            
            % Total packets to walk through: 
            totalPacketsToRead = segmentDataPoints(currSegment);
            
            % Bytes to read: (totalPacketsToRead * packetSize)
            % This might be large (e.g. 50GB). We should check if we can fit in RAM?
            % User has 128GB RAM. 50GB file fits.
            % However, to be safe(r), we can do this in large chunks (e.g. 1GB).
            ...
            while currPacketIdx <= totalPacketsToRead
...
if flagReadData && flagOneSamplePerPacket
    if flagAlign
        for ii = 1:length(NSx.Data)
            fprintf('  [Profile] Aligning Segment %d...\n', ii);
            t_align_start = tic;
            ...
            % Calculate the number of samples that should be added or removed
