function TSLloadNevPause(filepath)
% clc, clear, close all
...
for thisTrial = 1:NumberTotalTrials
    ...
    if sum(idx) ~= 0
        RawExpmark(1,thisTrial) = timeinthisTrial(idx);
    end
    ...
    if sum(idx) >=2
        ...
        RawExpmark(2,thisTrial) = CondID;
    end
    idx = eventsinthisTrial == 96; % reward event
    if sum(idx) ~= 0;
        RawExpmark(3,thisTrial) = timeinthisTrial(idx); % reward time
    end
    % Expmark (4,:) is the time of END_POST_TRIAL
    idx = eventsinthisTrial == end_post_trial;
    if sum(idx) ~=0
        RawExpmark(4,thisTrial) = timeinthisTrial(idx);
    end
    ...
    for testnp1 = 1:numel(turn_testnp1_on)
        if ismember(turn_testnp1_on(testnp1),eventsinthisTrial)
            ...
            RawExpmark(testnp1+4,thisTrial) = timeinthisTrial(idx);
        end
    end
    ...
    for testnp1 = 1:numel(turn_testnp1_off)
        if ismember(turn_testnp1_off(testnp1),eventsinthisTrial)
            ...
            RawExpmark(testnp1+9,thisTrial) = timeinthisTrial(idx);
        end
    end
end
...
save(matpath,'ExpmarkST','ExpmarkTime','RawExpmark')
matpath = [dirpath,slash,'Waveform.mat'];
save(matpath,'Waveform','-v7.3')
