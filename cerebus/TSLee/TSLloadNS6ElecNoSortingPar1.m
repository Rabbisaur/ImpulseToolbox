...
elecgrouping = ones(elecgroupingmember,1) * NumberPar;
if elecgroupingremainder
    elecgrouping = [elecgrouping;elecgroupingremainder];
end

GroupElectrodesOrder = mat2cell(ElecOrder,elecgrouping,1);
GroupElectrodeNumber = mat2cell((1:NsxBasic.NumElec)',elecgrouping,1);


for thisElecGroup = 1:NumElecgroups
    disp(['Processing electrode group #' num2str(thisElecGroup)]);
    disp(['with members:'])
    disp(GroupElectrodesOrder{thisElecGroup}')
    fseek(NsxID,DataStartFilepos,-1);
    NumElecInGroup = numel(GroupElectrodeNumber{thisElecGroup});
    RawSpikeStamps = cell(NumElecInGroup,1);
    RawWaveform = cell(NumElecInGroup,1);
    threshold = zeros(NumChunks,NumElecInGroup);
    % initialize LFP
    rawLFP = cell(NumElecInGroup,1);
    rawMUA = cell(NumElecInGroup,1);
    ...
    for thisChunk = 1:NumChunks
        ...
        for thisElec = 1:NumElecInGroup
            rawLFP{thisElec} = [rawLFP{thisElec};LFPChunk(1:SampleRate/LFPSampleRate:size(LFPChunk,1),thisElec)];
        end
        % half wave rectify in the negative direction
        MUAChunk = MUAChunk.*(MUAChunk<0);
        meanMUAChunk = mean(MUAChunk,1);
        stdMUAChunk = std(MUAChunk,0,1);
        ...
        MUAChunk = MUAChunk < repmat(MUAthres,size(MUAChunk,1),1);
        
        
        for thisElec = 1: NumElecInGroup
            rawMUAElec = find(MUAChunk(:,thisElec));
            rawMUAElec = histc(rawMUAElec,edge);
            rawMUAChunk = rawMUAElec(1:NumMillisec,:);
            rawMUA{thisElec} = [rawMUA{thisElec};rawMUAChunk];
        end
        ...
        for thisElec = 1:NumElecInGroup
            % Spikestamps
            Spikes = find(C_poz_diff(:,thisElec) == 1);
            ...
            ElecDataChunk = DataChunk(:,thisElec);
            % get waveforms
            ...
            C_waveIndex = repmat(C_wavestart,1,waveformLength) + repmat(0:(waveformLength-1),numel(C_wavestart),1);
            tempWF = reshape(ElecDataChunk(C_waveIndex),[],waveformLength);
            
            % store spikes and waveforms
            RawSpikeStamps{thisElec} = [RawSpikeStamps{thisElec}; Spikes + Timestamp + datachunksize* SampleRate * (thisChunk-1)];
            RawWaveform{thisElec} = [RawWaveform{thisElec};tempWF];
        end
    ...
    for thisElec = 1:NumElecInGroup
        ...
        elecRawSpikeStamps = RawSpikeStamps{thisElec};
        save([savedir,slash, 'RawSpikeStamps6.mat'],'elecRawSpikeStamps');
    end
    disp(['Finished with electrode group #' num2str(thisElecGroup)])
end
