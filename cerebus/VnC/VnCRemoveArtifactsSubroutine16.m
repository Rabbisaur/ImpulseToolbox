function [ARdata,ArtefactLocation,Interpolateddata] = VnCRemoveArtifactsSubroutine16(RawTrace,StimulationParam,StimStartTime,SampleRate)
% Exponential fitting on data fragments
...
%     figure
    for thisArtefect = 2:(numberofStimPulses-2)
        % perform exp fiting for each piece of data
        yidx = (idx1(end,thisArtefect)+1):(idx1(1,thisArtefect+1)-1);
        yy = y(yidx);
%         subplot(1,3,1)
%         ylim([-2500 2500])
%         hold on
...
%         yy = yy - f2(xx);
        ...
        x = (0:1/30000:(1/30000*(numel(yy)-1)))';
        f = fit(x,yy,'exp1','StartPoint',[1,1]);
%         hold off
%         plot(x,yy),hold on, plot(x,f(x))
        yy = yy - f(x);
        y(yidx) = yy;
    end
%     y(isnan(y)) = 0;
    ...
    ARdata(1:200,thisTrial) = 0;
    ARdata(end-199:end,thisTrial) = 0;
end
