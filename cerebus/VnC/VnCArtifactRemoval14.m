function instanceinfo = VnCArtifactRemoval14(instanceinfo,StimulationParam,MUAparameters,trialParam,trialAlignSTfieldname,interpolationLength,GoodTrialIdx,GoodMStrialIdx,ARFineAlignFlag,linenoiseFreq)
...
interpolationLengthDPupperLimit = round((1/StimulationParam.PulsesFrequency)*1/2 * SampleRate);
...
for thisInstance = 1:numInstances
    ...
    for thisElec = 1:min([numElec,128])
        ...
        EnoughMemoryFlag= 0;
        CheckCount = 0;
        while EnoughMemoryFlag==0
            CheckCount = CheckCount + 1;
            [~,systemview] = memory;
            if systemview.PhysicalMemory.Available > 8*1024*1024*1024
                EnoughMemoryFlag = 1;
                fprintf('\n')
            else
                if CheckCount == 1
                    fprintf('Waiting for memory')
                else
                    fprintf('.')
                end
                pause(2)
        ...
        [trialData, AllArtifactTimepoints,Interpolateddata,WaveDPbefore,WaveDPafter] = VnCRemoveArtifactsSubroutine13(trialData,StimulationParam,StimStartTime,SampleRate,EID,thisInstance,instanceinfo,GoodMStrialIdx,ARFineAlignFlag);
        
        % search for best interpolation length
        BPdata = [];
        [baselineMUAe, BPdata] = GetMUAewithInterpolationSubroutine3(trialData0(:,GoodTrialIdx),SampleRate,MUAparameters,AllArtifactTimepoints,0,BPdata);
        ...
        for interpolationLengthDP = round(WaveDPbefore+WaveDPafter):interpolationLengthDPupperLimit
            ...
            [testMUAe,BPdata] = GetMUAewithInterpolationSubroutine3(trialData(:,GoodTrialIdx),SampleRate,MUAparameters,AllArtifactTimepoints,interpolationLength,BPdata);
        ...
        MUAe = GetMUAewithInterpolationSubroutine2(trialData,SampleRate,MUAparameters,AllArtifactTimepoints,interpolationLength,linenoiseFreq);
        ...
        MUAe = GetMUAewithInterpolationSubroutine2(trialData,SampleRate,MUAparameters,AllArtifactTimepoints,electrodeArtifactRemovedNeuralData.MUAeMeanLevelInterpolationLength,linenoiseFreq);
        MUAe.time =  (0:(size(MUAe.data,1)-1))/MUAe.MUAesamplerate + trialParam.startT;
