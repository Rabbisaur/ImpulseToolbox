function [ARdata,ArtifactLocation,Interpolateddata,WaveDPbefore,WaveDPafter] = VnCRemoveArtifactsSubroutine13(RawTrace,StimulationParam,StimStartTime,SampleRate,EID,thisInstance,instanceinfo,GoodMStrialIdx,FineAlignFlag)
...
DecayCompensationLength = 0.0001; % sec
...
for thisTrial = 1:NumTrials
    ...
    % perform Salpa here
%     for thisArtefect = (NumPulsesBefore-1):(NumPulsesBefore + StimulationParam.numberofStimPulses + 1)
    yy = y(1:idx(1,1));
    yy = salpa(yy,'tau',round(tau*SampleRate*SSrate));
    y(1:idx(1,1)) = yy;
    for thisArtefect = 1:(numberofStimPulses-1)
%         diffval = y(idx(1,thisArtefect)) - y(idx(end,thisArtefect));
% %         extrapolateVal = (y(idx(end,thisArtefect)) - y(idx(end,thisArtefect) + round(DecayCompensationLength*SSrate*SampleRate))) / (DecayCompensationLength*SSrate*SampleRate) * size(idx,1);
% %         diffval = diffval - extrapolateVal;
%         y(idx(end,thisArtefect):end) = y(idx(end,thisArtefect):end) + diffval;
        yidx = (idx(end,thisArtefect)+1):(idx(1,thisArtefect+1)-1);
        yy = y(yidx);
        % extrapolation on both end
%         Hsize = round((tau)*30000*SSrate);
        Csize = numel(yy);
%         x = Hsize:(Csize+Hsize-1);
%         xx = 1:(Csize+2*Hsize);
%         yy = interp1(x,yy,xx,'linear','extrap');
