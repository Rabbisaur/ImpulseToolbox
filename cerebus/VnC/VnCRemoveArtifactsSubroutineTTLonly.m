function [ARdata,ArtifactLocation,Interpolateddata,WaveDPbefore,WaveDPafter] = VnCRemoveArtifactsSubroutineTTLonly(RawTrace,StimulationParam,StimStartTime,SampleRate,EID,thisInstance,instanceinfo,GoodMStrialIdx,FineAlignFlag)
...
for thisTrial = 1:NumTrials
    ...
    % perform one extra step, level off the trace
    for thisArtefect = 1:size(idx,2)
        diffval = y(idx(1,thisArtefect)) - y(idx(end,thisArtefect));
        % also take slop into account
        y1 = y(idx(end,thisArtefect));
        y2 = y(idx(end,thisArtefect)+slopeLengthValue*SSrate*SampleRate);
        dx1 = slopeLengthValue*SSrate*SampleRate;
        dx2 = size(idx,1);
        diffval = diffval - (y1-y2)/dx1 * dx2;
    ...
    ydata = downsample(ydata,SSrate);
    plot(ydata,'black');
    Interpolateddata(:,thisTrial) = interp1(x,y,xx,'spline');
%     Interpolateddata(:,thisTrial) = interp1(x,y,xx,'nearest');
    waveform = Interpolateddata(idx,thisTrial);
    waveform = reshape(waveform',[],numberofStimPulses);
    ydata = waveform(:,idx2)-mean([waveform(1,idx2),waveform(end,idx2)]);
    ydata = downsample(ydata,SSrate);
