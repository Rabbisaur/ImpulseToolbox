function instanceinfo = VnCMicroStimTrialAlignRef2(instanceinfo,matdatapath,StimulationParam,RefCh,trialParam,UsingTTLflag,UsingMonitorFlag,electrodeFlag,whichmethod)
...
if UsingTTLflag == 1
    ...
    if RefCh(2,1) == 0 % use sum of all channels
        ...
        arrayRefCh = RefCh(2,2) - 128;
    end
    trialAlignST = instanceinfo(thisInstance).trialInfo.validTrialAlignST;
    for thisElec = 1:numel(arrayRefCh)
        EID = 128+arrayRefCh(thisElec);
        electrodeTrialData = VnCGetTrialData2(instanceinfo, thisInstance, EID, trialAlignST, trialParam);
        if thisElec == 1
            AllElecData = electrodeTrialData;
        else
            AllElecData = AllElecData + electrodeTrialData;
        end
    end
else
