function instanceinfo = VnCArtifactRemoval3(instanceinfo,StimulationParam,CurrentLevel,MUAparameters,LFPparameters,trialParam,trialAlignSTfieldname,interpolationLength,DebugFlag)
...
%             DebugFlag = 1;
            [CondTrialData, artifactTimepoints] = VnCRemoveArtifactsSubroutine2(CondTrialData,StimulationParam,trialParam,CondStimStartEndTime,SampleRate,SupersamplingRatio,DebugFlag);
            AllArtifactTimepoints(:,idx) = artifactTimepoints;

            trialData(:,idx) = CondTrialData;
        end
        % calculate MUA, LFP
        [MUAe, LFP] = GetMUAeLFPwithInterpolationSubroutine2(trialData,SampleRate,MUAparameters,AllArtifactTimepoints,interpolationLength,LFPparameters);
