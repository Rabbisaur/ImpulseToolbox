function instanceinfo = VnCfindTrials3(instanceinfo,matdatapath,TrialBit,AlignBit)
...
%             currentPoz = currentPoz + find(instanceinfo(thisInstance).nev.digidata(currentPoz : end) == trialCode,1,'first') - 1;
        end
    end
...
if xingFlag
    ...
    if  NumBadinstance > 0
        ...
        for thisBadInstance = 1:NumBadinstance
            ...
            instanceinfo(InstanceIdx).trialInfo.trialAlignST(idx) = instanceinfo(goodInstanceIdx).trialInfo.trialAlignST(idx) + STdiff;
            idx = find(idx);
