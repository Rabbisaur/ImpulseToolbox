function instance = VnCloadcerebusdata4(sessionpath,instanceRange,instance,linenoiseFreq)
fclose('all');
cacheDir = '';
...
%     for thisfile = 6
    try
        ...
        instance(thisfile).nev.DigitalFactor = nevdata.ElectrodesInfo.DigitalFactor;
        ...
        instance(thisfile).nev.NsxExtend = NsxExtend;
...
%         [~,trialIdx] = max(Trial.NumDP);
    ...
    for thisElec = 1:NumValidElec % close all disk cache files
        fseek(elecfp(thisElec),0,'bof');
        tmpdata = fread(elecfp(thisElec),inf,'int16=>double');
        FsD = SampleRate;
        Fn = FsD/2; % Downsampled Nyquist frequency
        for v = [linenoiseFreq 2*linenoiseFreq 3*linenoiseFreq]
            Fbp = [v-2,v+2];
            [Blp, Alp] = butter(2, [min(Fbp)/Fn max(Fbp)/Fn],'stop'); % compute filter coefficients
            tmpdata = filtfilt(Blp, Alp, tmpdata);
        end
        fseek(elecfp(thisElec),0,'bof');
        tmpdata = round(tmpdata);
        tmpdata = int16(tmpdata);
        fwrite(elecfp(thisElectrode),tmpdata,'int16');
    end
