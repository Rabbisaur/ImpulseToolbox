function instanceinfo = VnCArtifactRemoval11(instanceinfo,StimulationParam,MUAparameters,LFPparameters,trialParam,trialAlignSTfieldname,interpolationLength,DebugFlag)
disp('Removing stimulation artifacts...')

matversion = '-v6';
interpolationLengthDPupperLimit = 30;
...
for thisInstance = 1:numInstances
    ...
    for thisElec = 1:numElec
        ...
        [trialData, AllArtifactTimepoints,Interpolateddata,WaveDPbefore,WaveDPafter] = VnCRemoveArtifactsSubroutine9(trialData,StimulationParam,StimStartTime,SampleRate);
        
        % search for correct interpolation length
        baselineMUAe = GetMUAewithInterpolationSubroutine(trialData0,SampleRate,MUAparameters,AllArtifactTimepoints,0);
        baselineMUAe.time =  (0:(size(baselineMUAe.data,1)-1))/baselineMUAe.MUAesamplerate + trialParam.startT;
        idx = baselineMUAe.time < 0;
        meanBaseline = mean(mean(baselineMUAe.data(idx,:),2));
        
        counter = 0;
        meanTest = [];
        for interpolationLengthDP = 0:interpolationLengthDPupperLimit
            counter = counter + 1;
            interpolationLength = (WaveDPafter + WaveDPbefore+ interpolationLengthDP) /SampleRate;
            testMUAe = GetMUAewithInterpolationSubroutine(trialData,SampleRate,MUAparameters,AllArtifactTimepoints,interpolationLength);
            testMUAe.time =  (0:(size(testMUAe.data,1)-1))/testMUAe.MUAesamplerate + trialParam.startT;
            idx = testMUAe.time < 0;
            meanTest(counter) = mean(mean(testMUAe.data(idx,:),2));
        end
        meanDiff = abs(meanTest - meanBaseline);
        [~,idx] = min(meanDiff);
        interpolationLengthDP = 0:interpolationLengthDPupperLimit;
        interpolationLengthDP = interpolationLengthDP(idx);
        disp(['interpolationLengthDP = ',num2str(interpolationLengthDP)])
