function [ARdata,ArtifactLocation] = VnCRemoveArtifactsSubroutine4(RawTrace,StimulationParam,StimStartEndTime,SampleRate,SupersamplingRatio,debug)
...
% HighPassdata = filtfilt(Bhigh,Ahigh,RawTrace);
HighPassdata = RawTrace;
BandPassdata = filtfilt(Bartifact,Aartifact,RawTrace);
% HighPassdata = HighPassdata * Gain;
BandPassdata = filtfilt(Bpulse,Apulse,BandPassdata);
if size(RawTrace,2) > size(RawTrace,1)
    RawTrace = RawTrace';
end
if size(BandPassdata,2) > size(BandPassdata,1)
...
for thisDirection = 1:2
    ...
    for thisTrial = 1:NumTrials
        trialdata = BandPassdata(:,thisTrial);
...
for thisTrial = 1:NumTrials
    ...
    idx = repmat(idx,1,numel(loc));
    if size(loc,1)>size(loc,2) % row
        loc = loc';
    end
    loc = repmat(loc,numinterpDP,1);
    idx = idx + loc;
    
    % linear interpolate
    y = HighPassdata(:,thisTrial);
    x = 1:numel(y);
    xx = x;
    x(idx) = [];
    y(idx) = [];
    ...
    y = salpa(yy,'tau',round(0.001*SampleRate*SupersamplingRatio));
    
    
    
    nanidx = isnan(y);
    y(nanidx) = HighPassdata(nanidx,thisTrial);
    
%     % mirror data
%     idx1 = idx(1:floor(numinterpDP/2),:);
%     idx2 = idx1 - floor(numinterpDP/2);
%     idx3 = idx((ceil(numinterpDP/2)+1):end,:);
%     idx4 = idx3 + floor(numinterpDP/2);
%     y(idx1) = y(flipud(idx2));
%     y(idx3) = y(flipud(idx4));
...
%         loc(thisLoc) = loc2(thisLoc) + idx;
%     end
    loc(thisLoc) = round(mean([loc1(thisLoc),loc2(thisLoc)]));
end
end
