classdef PlotUtils
    methods(Static)
        ...
        function plotOverChannels(segmentTensor, trialIdx, pulseIdx)
            ...
            tvec = PlotUtils.lindelta(0, 1/30, pulseLen);
            PlotUtils.pt(2, tvec, segmentTensor(trialIdx, :, pulseIdx, :));
            axis tight;
            axis off;
        end
        ...
        function plotOverTrials(segmentTensor, pulseIdx, chIdx)
            pulseLen = size(segmentTensor, 2);
            tvec = PlotUtils.lindelta(0, 1/30, pulseLen);
            PlotUtils.pt(2, tvec, segmentTensor(:, :, pulseIdx, chIdx), 'alpha', 1);
            axis tight;
            axis off;
        end
        ...
        function plotBeforeAfterOverTrials(before, after, pulseIdx, chIdx)
            ...
            tvec = PlotUtils.lindelta(0, 1/30, pulseLen);
            PlotUtils.pt(2, tvec, before(:, :, pulseIdx, chIdx), 'alpha', 1);
            hold on
            pulseLen = size(after, 2);
            tvec = PlotUtils.lindelta(0, 1/30, pulseLen);
            PlotUtils.pt(2, tvec, after(:, :, pulseIdx, chIdx), 'alpha', 1, 'Color', 'b');
            axis tight;
            axis off;
        end
        ...
        function plotBeforeAfterOverTrialsAllChannels(before, after, pulseIdx)
            ...
            for c = 1:nChannels
                PlotUtils.subtightplot(nRow, nCol, c);
                PlotUtils.plotBeforeAfterOverTrials(before, after, pulseIdx, c);
            end
        end
        ...
        function plotOverTrialsAllChannels(segmentTensor, pulseIdx)
            ...
            for c = 1:nChannels
                PlotUtils.subtightplot(nRow, nCol, c);
                PlotUtils.plotOverTrials(segmentTensor, pulseIdx, c);
            end
        end
        ...
        function plotOverTrain(segmentTensor, trialIdx, chIdx)
            ...
            pulseLen = size(segmentTensor, 2);
            tvec = PlotUtils.lindelta(0, 1/30, pulseLen);
            PlotUtils.pt(2, tvec, segmentTensor(trialIdx, :, :, chIdx), 'alpha', alpha);
            axis tight;
            axis off;
        end
        ...
        function plotOverTrainAllChannels(segmentTensor, trialIdx)
            ...
            for c = 1:nChannels
                PlotUtils.subtightplot(nRow, nCol, c);
                PlotUtils.plotOverTrain(segmentTensor, trialIdx, c);
                ax = gca;
            ...
            PlotUtils.equalizeAxisLimits(handles);
        end
        ...
        function plotBeforeAfterOverTrain(before, after, trialIdx, chIdx)
            ...
            pulseLen = size(before, 2);
            tvec = PlotUtils.lindelta(0, 1/30, pulseLen);
            PlotUtils.pt(2, tvec, before(trialIdx, :, :, chIdx), 'alpha', 1, 'Color', 'k');
            hold on

            pulseLen = size(after, 2);
            tvec = PlotUtils.lindelta(0, 1/30, pulseLen);
            PlotUtils.pt(2, tvec, after(trialIdx, :, :, chIdx), 'alpha', 1, 'Color', 'b');
            axis tight
            axis off;
        end
        ...
        function plotBeforeAfterOverTrainAllChannels(before, after, trialIdx)
            ...
            for c = 1:nChannels
                PlotUtils.subtightplot(nRow, nCol, c);
                PlotUtils.plotBeforeAfterOverTrain(before, after, trialIdx, c);
        ...
        function plotRawWithArtifactOverTrials(before, after, pulseIdx, chIdx)
            ...
            pulseLen = size(before, 2);
            tvec = PlotUtils.lindelta(0, 1/30, pulseLen);
            PlotUtils.pt(2, tvec, before(trialIdx, :, pulseIdx, chIdx), 'alpha', 1, 'Color', 'k');
            hold on
            pulseLen = size(after, 2);
            tvec = PlotUtils.lindelta(0, 1/30, pulseLen);
            PlotUtils.pt(2, tvec, after(trialIdx, :, pulseIdx, chIdx), 'alpha', 1, 'Color', 'r');
            axis tight;
            axis off;
        end
        ...
        function plotRawWithArtifactOverTrialsAllChannels(before, after, pulseIdx)
            ...
            for c = 1:nChannels
                PlotUtils.subtightplot(nRow, nCol, c);
                PlotUtils.plotRawWithArtifactOverTrials(before, after, pulseIdx, c);
        ...
        function plotRawWithArtifactOverPulses(before, artifact, trialIdx, chIdx)
            ...
            tvec = PlotUtils.lindelta(0, 1/30, pulseLen);
            PlotUtils.pt(2, tvec, artifact(trialIdx, :, pulseIdx, chIdx), 'alpha', 1, 'Color', 'r');
        ...
        function plotRawWithArtifactOverPulsesAllChannels(before, artifact, trialIdx)
            ...
            for c = 1:nChannels
                PlotUtils.subtightplot(nRow, nCol, c);
                PlotUtils.plotRawWithArtifactOverPulses(before, artifact, trialIdx, c);
        ...
        function [h, cmap] = pt(timeDim, varargin)
            ...
            if isvector(varargin{1}) && narg > 1 && isnumeric(varargin{2})
                x = varargin{2};
                tvec = TensorUtils.makecol(varargin{1});
                args = varargin(3:end);
            else
                x = varargin{1};
            ...
            if isempty(p.Results.coloreval)
                ...
                colors = PlotUtils.evalColorMapAt(cmap, coloreval);
