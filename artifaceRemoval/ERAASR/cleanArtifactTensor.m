...
% dataTensor is nTrials x extractWindowDuration x nChannels
    ...
    p.addParameter('nPC_channels', 4, @isscalar);
    p.addParameter('nPC_trials', 4 , @isscalar);
    p.addParameter('nPC_pulses', 2, @isscalar);

    p.addParameter('omit_bandwidth_channels', 1, @isscalar);
    ...
    p.addParameter('cleanOverChannelsIndividualTrials', true, @islogical);
    p.addParameter('cleanOverPulsesIndividualChannels', false, @islogical);
    p.addParameter('cleanOverTrialsIndividualChannels', false, @islogical);
    
    p.addParameter('cleanPostStim', false, @islogical);
    ...
    if upsample > 1
        ...
        dataTensor = Utils.resampleTensorInTime(dataTensor, 2, tvec, 'timeDelta', 1/upsample);
    end
    ...
    if p.Results.hpCornerHz > 0
        ...
        dataTensor = highPassFilter(dataTensor, Fs, 'cornerHz', p.Results.hpCornerHz, ...
    ...
    if showFigures
        for iR = 1:numel(plotTrialIdx)
            for iP = 1:numel(plotPulseIdx)
                ...
                PlotUtils.plotOverChannels(segmentTensorRaw, 1, 1);
                saveFig();
            end
        end
    end
    ...
    if nPC_channels > 0
        ...
        if p.Results.cleanOverChannelsIndividualTrials
            ...
            pcaMatOverTrials = TensorUtils.reshapeByConcatenatingDims(segmentTensor, {[2 3], 4, 1});
            pcaArt = nan(size(pcaMatOverTrials));

            pcaCleaned = nan(size(pcaMatOverTrials));
            
            if ~quiet, prog = Utils.ProgressBar(nTraces, 'Cleaning over channels for each individual trial'); end
            for r = 1:nTraces
                if ~quiet, prog.update(r); end
                pcaMat = pcaMatOverTrials(:, :, r); % TP x C
                [pcaCleaned(:, :, r), artPcs, pcaArt(:, :, r)] = cleanMatrixViaPCARegression(pcaMat, nPC_channels, ...
                    'omitAdjacentChannelsBandWidth', obw_channels, 'pcaOnlyOmitted', p.Results.pcaOnlyOmitted);
            end
            if ~quiet, prog.finish(); end
            segmentTensor = TensorUtils.undoReshapeByConcatenatingDims(pcaCleaned, {[2 3], 4, 1}, size(segmentTensor));

            % reconstruct artifact detection into segmentTensor shape
            tensorArtifact = TensorUtils.undoReshapeByConcatenatingDims(pcaArt, {[2 3], 4, 1}, size(segmentTensor));
        else
            % clean using PCA + regression
            % RTP x C --> each PC is RTP x 1
            pcaMat = TensorUtils.reshapeByConcatenatingDims(segmentTensor, {[2 3 1], 4});
            [pcaCleaned, artPcs, artMat] = cleanMatrixViaPCARegression(pcaMat, nPC_channels, ...
                'omitAdjacentChannelsBandWidth', obw_channels, 'pcaOnlyOmitted', p.Results.pcaOnlyOmitted);
            segmentTensor = TensorUtils.undoReshapeByConcatenatingDims(pcaCleaned, {[2 3 1], 4}, size(segmentTensor));

            % reconstruct artifact detection into segmentTensor shape
            tensorArtifact = TensorUtils.undoReshapeByConcatenatingDims(artMat, {[2 3 1], 4}, size(segmentTensor));
        end
        ...
        if showFigures
            ...
            PlotUtils.plotStackedPCTraces(pcsFirstTrial(:, 1:min(6, size(pcsFirstTrial,2))));
            saveFig();

            for iR = 1:numel(plotTrialIdx)
                figSetName('channels_prePost_overPulses_trial%d', plotTrialIdx(iR));
                PlotUtils.plotBeforeAfterOverTrainAllChannels(segmentTensorCached, segmentTensor, plotTrialIdx(iR));
                saveFig();
            ...
            for iP = 1:numel(plotPulseIdx)
                ...
                PlotUtils.plotRawWithArtifactOverTrialsAllChannels(segmentTensorCached, tensorArtifact, plotPulseIdx(iP));
                saveFig();
            end
        end
    end
    ...
    if p.Results.alignPulsesOverTrain
        ...
        delays = Utils.findDelaysTensor(segmentTensorRaw, ...
        ...
        segmentTensor = Utils.removeDelaysTensor(segmentTensor, delays, ...
    ...
    if nPC_pulses > 0
        ...
        if p.Results.cleanOverPulsesIndividualChannels
            ...
            pcaMatOverChannels = TensorUtils.reshapeByConcatenatingDims(segmentTensor, {[2 1], 3, 4});
            ...
            segmentTensorPartial = TensorUtils.undoReshapeByConcatenatingDims(pcaCleaned, {[2 1], 3, 4}, size(segmentTensorPartial));

            % reconstruct artifact detection into segmentTensor shape
            tensorArtifact = TensorUtils.undoReshapeByConcatenatingDims(pcaArt, {[2 1], 3, 4}, size(segmentTensorPartial));

        else
            % TRC x P --> each PC is TRC x 1, weights are P x 1,
            pcaMat = TensorUtils.reshapeByConcatenatingDims(segmentTensorPartial, {[2 1 4], 3});
            [pcaCleaned, ~, pcaArt] = cleanMatrixViaPCARegression(pcaMat, nPC_pulses, ...
                'omitAdjacentChannelsBandWidth', obw_pulses, 'pcaOnlyOmitted', p.Results.pcaOnlyOmitted);

            segmentTensorPartial = TensorUtils.undoReshapeByConcatenatingDims(pcaCleaned, {[2 1 4], 3}, size(segmentTensorPartial));

            tensorArtifact = TensorUtils.undoReshapeByConcatenatingDims(pcaArt, {[2 1 4], 3}, size(segmentTensorPartial));
        end
        ...
        if showFigures
            for iR = 1:numel(plotTrialIdx)
                ...
                PlotUtils.plotRawWithArtifactOverPulsesAllChannels(segmentTensorCache, tensorArtifact, plotTrialIdx(iR));
                saveFig();
            ...
            for iP = 1:numel(plotPulseIdx)
                ...
                PlotUtils.plotOverTrialsAllChannels(segmentTensor, plotPulseIdx(iP));
                saveFig();
            end
        end
    end
    ...
    if nPC_trials > 0
        ...
        if p.Results.cleanOverTrialsIndividualChannels
            % TP x R x CPP
            pcaMatOverChannels = TensorUtils.reshapeByConcatenatingDims(segmentTensor, {[2 3], 1, 4});
            pcaArt = nan(size(pcaMatOverChannels));

            pcaCleaned = nan(size(pcaMatOverChannels));
%             if ~quiet, prog = Utils.ProgressBar(nChannels, 'Cleaning over trials for each individual channel'); end
            for c = 1:nChannels
                if ~quiet, prog.update(c); end
                pcaMat = pcaMatOverChannels(:, :, c); % TP x R
                [pcaCleaned(:, :, c), ~, pcaArt(:, :, c)] = cleanMatrixViaPCARegression(pcaMat, nPC_trials, ...
            ...
            tensorArtifact = TensorUtils.undoReshapeByConcatenatingDims(pcaArt, {[2 3], 1, 4}, size(segmentTensor));

        else
            % originally R x T x P x C
            % TPC x R --> each PC is TPC x 1, weights are R x 1,
            pcaMat = TensorUtils.reshapeByConcatenatingDims(segmentTensor, {[2 3 4], 1});
            [pcaCleaned, ~, pcaArt] = cleanMatrixViaPCARegression(pcaMat, nPC_trials, ...
                'omitAdjacentChannelsBandWidth', obw_trials, 'pcaOnlyOmitted', p.Results.pcaOnlyOmitted);
        ...
        if showFigures
            for iP = 1:numel(plotPulseIdx)
                ...
                PlotUtils.plotRawWithArtifactOverTrialsAllChannels(segmentTensorCached, tensorArtifact, 1);
                saveFig();

                figSetName('trials_beforeAfter_overTrials_pulse%d', plotPulseIdx(iP));
                PlotUtils.plotBeforeAfterOverTrialsAllChannels(segmentTensorCached, segmentTensor, plotPulseIdx(iP));
                saveFig();

                figSetName('trials_cleaned_overTrials_pulse%d', plotPulseIdx(iP));
                PlotUtils.plotOverTrialsAllChannels(segmentTensor, 1);
                saveFig();
            end
    ...
    if nPC_channels > 0 && p.Results.cleanPostStim
        ...
        % RT x C
        pcaMat = TensorUtils.reshapeByConcatenatingDims(dataPostStim, {[1 2] 3});
        pcaCleaned = cleanMatrixViaPCARegression(pcaMat, nPC_channels, ...
            'omitAdjacentChannelsBandWidth', obw_channels, 'pcaOnlyOmitted', p.Results.pcaOnlyOmitted);
        dataPostStim = TensorUtils.undoReshapeByConcatenatingDims(pcaCleaned, {[1 2], 3}, size(dataPostStim));
    ...
    if upsample > 1
        ...
        dataTensor = Utils.resampleTensorInTime(dataTensor, 2, tvec, 'timeDelta', 1);
    end
