function pattern = polargratings2(sizeX,sizeY,meanLum,contrast,radius,concentricspatialfreq,radialspatialfreq,phase,supersampling,bgLum,maskflag,maskRadius,maskstd,maskonedgeflag,edgemaskRadius,edgemaskstd,edgemasklum)
...
L1 = @(x,y)cos(pi.*concentricspatialfreq(1).*sqrt(x.^2+y.^2)+radialspatialfreq(1).*atan2(y,x)+phase(1));
...
if numel(phase) == 2
    L2 = @(x,y)cos(pi.*concentricspatialfreq(2).*sqrt(x.^2+y.^2)+radialspatialfreq(2).*atan2(y,x)+phase(2));
    idx = rr > radius;
    pattern(idx) = L2(xx(idx),yy(idx));
end

% % L = @(x,y)cos(2*pi*concentricspatialfreq*sqrt(x^2+y^2)+polphase)+cos(radialspatialfreq*atan2(y,x)+radphase);
% L1 = @(x,y)cos(2*pi*concentricspatialfreq(1)*sqrt(x^2+y^2)+radialspatialfreq(1)*atan2(y,x)+phase(1));
% L2 = @(x,y)cos(2*pi*concentricspatialfreq(2)*sqrt(x^2+y^2)+radialspatialfreq(2)*atan2(y,x)+phase(2));
%
