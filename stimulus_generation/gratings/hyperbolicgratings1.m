function pattern = hyperbolicgratings(sizeX,sizeY,meanLum,contrast,frequency,orientation,phase,supersampling,bgLum,maskflag,maskRadius,maskstd)
...
L = @(x,y)cos(2*pi*frequency*abs(sqrt((x*cos(orientation)-y*sin(orientation))*(x*sin(orientation)+y*cos(orientation))))+phase);
